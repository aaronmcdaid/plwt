namespace polymorphic_lambdas_with_typeof {
...
#define POLYMORPHIC_LAMBDA_WITH_TYPEOF(lambda_text) { lambda_text } ) lambda_type ; }

#define PLWT( name, t0, t1)  \
	template <typename T0, typename T1> \
	struct just_the_lambda_itself_ ## name; \
	template <typename T0, typename T1> \
	static auto name(T0 && t0, T1 && t1) -> typename polymorphic_lambdas_with_typeof :: all_the_other_details<T0,T1,  just_the_lambda_itself_ ## name<T0,T1> > :: return_type \
	{ \
		return  polymorphic_lambdas_with_typeof ::all_the_other_details<T0,T1,  just_the_lambda_itself_ ## name<T0,T1> > :: execute(std::forward<T0>(t0), std::forward<T1>(t1)); \
	} \
	template <typename T0, typename T1> \
	struct just_the_lambda_itself_ ## name { \
		/* don't ask my why, but typeof work here where decltype doesn't. I suppose that ties me to g++ for now. */ \
		typedef typeof( [](const T0 & t0,const T1 & t1) POLYMORPHIC_LAMBDA_WITH_TYPEOF
